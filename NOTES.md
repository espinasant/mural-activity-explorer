...
Given the ~4-hour guidance, I prioritized a small set of features that demonstrate end-to-end thinking:

1. **Load and render** sticky notes at their board coordinates
2. **Pan and zoom** so users can navigate a large canvas
3. **Filter** by author and color
4. **Scrub a timeline** to see how the board evolved over time
5. **Show stats** (notes per author / color)

Implementation order was backend-first (data + API contract), then the board viewport, then filters and timeline.
...
- **Timestamps:** The provided schema in the challenge's description had no `createdAt`, so I added one to enable timeline filtering. All notes in the dataset include it.
...
- **Layered Architecture:** `routes → services → repository` keeps HTTP concerns separate from data access and makes the service layer easy to swap (JSON file today, DB tomorrow).
...
- **Data fetching:** A lightweight `useQuery` hook fetches notes and stats on mount.
...
Yes — I used **Cursor** (AI-assisted IDE) throughout the project:

- **Scoping and architecture:** Brainstorming the hybrid filtering model, API contract, and folder structure before writing code.
...
| Global stats (not filter-scoped)      | Simpler API and UI for the time box                   | Stats don't update when filters change — can feel inconsistent                              |
| Bottom bar vs sidebar                 | Keeps canvas maximized                                | Less room for rich analytics panels                                                         |
...
*(Optional — add your thoughts before submitting)*